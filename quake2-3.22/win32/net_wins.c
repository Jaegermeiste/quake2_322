...
void NetadrToSockadr (netadr_t *a, struct sockaddr *s)
{
	...
	{
		((struct sockaddr_in *)s)->sin_family = AF_INET6;
		((struct sockaddr_in *)s)->sin_addr.s_addr = *(int *)&a->ip;
		((struct sockaddr_in *)s)->sin_port = a->port;
	}
...
int	NET_GetLoopPacket (netsrc_t sock, netadr_t *net_from, sizebuf_t *net_message)
{
	int		i;
	loopback_t	*loop;
...
//=============================================================================
extern client_t *GetClientFromAdr (netadr_t address);
...
int	NET_GetPacket (netsrc_t sock, netadr_t *net_from, sizebuf_t *net_message)
{
	...
	for (protocol = 0 ; protocol < 2 ; protocol++)
	{
		...
		if (ret == -1)
		{
			...
			// Jitspoe bug fix: WSAECONNRESET 
			Com_Printf ("NET_GetPacket: %s from %s, dropping client\n", NET_ErrorString(), NET_AdrToString(*net_from)); 
			// Knightmare - drop this client so as not to flood the console 
...
void NET_SendPacket (netsrc_t sock, int length, void *data, netadr_t to)
{
	...
	else if (to.type == NA_MULTICAST6)	// NeVo - IPV6
	{
		net_socket = ip_sockets[sock];
		if (!net_socket)
			return;
...
void NET_OpenIP (void)
{
	...
	if (!ip_sockets[NS_SERVER])
	{
		...
		if (!port)
		{
			...
			if (!port)
			{
				if (!cl_randomport->value)
					port = Cvar_Get("sv_port", va("%i", PORT_SERVER), CVAR_ARCHIVE)->value;
				else
					port = Cvar_Set("sv_port", va("%1", (((rand()%11000) + 5000) * ((rand()%11000) + 5000)) / PORT_SERVER))->value;
			}
		}
	...
	if (!ip_sockets[NS_CLIENT])
	{
		...
		{
			if (!cl_randomport->value)
				port = Cvar_Get("clientport", va("%i", PORT_CLIENT), CVAR_ARCHIVE)->value;
			else
				port = Cvar_Set("clientport", va("%1", (((rand()%11000) + 5000) * ((rand()%11000) + 5000)) / PORT_CLIENT))->value;
			if (!port)
				port = PORT_ANY;
		}
