...
void VID_CreateWindow( int width, int height, int stylebits )
{
    ...
    wc.hIcon         = 0;
    //wc.hCursor       = LoadCursor (sww_state.hInstance, IDC_ARROW);
	// NeVo - get rid of windows cursor (transparent)
	wc.hCursor		 = CreateCursor (sww_state.hInstance, 0, 0, 1, 1, ANDmaskCursor, XORmaskCursor);
...
//			SelectPalette(hdcScreen, holdpal, FALSE);
	...
	else
	{
		RECT r;
		HRESULT rval;
		DDSURFACEDESC2 ddsd;	// NeVo - DirectDraw7 Graphics
