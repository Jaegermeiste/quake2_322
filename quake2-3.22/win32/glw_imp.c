...
static int GLimp_SwitchFullscreen( int width, int height );
...
int VID_CreateWindow( int width, int height, int fullscreen )
{
	WNDCLASS		wc;
	RECT			r;
    ...
    wc.hIcon         = 0;
    //wc.hCursor       = LoadCursor (glw_state.hInstance, IDC_ARROW);
	// NeVo - get rid of windows cursor (transparent)
	wc.hCursor		 = CreateCursor (glw_state.hInstance, 0, 0, 1, 1, ANDmaskCursor, XORmaskCursor);
	...
	AdjustWindowRect (&r, stylebits, 0);
...
** GLimp_SetMode
*/
rserr_t GLimp_SetMode( int *pwidth, int *pheight, int mode, int fullscreen )
{
	int width, height;
	...
	if ( fullscreen )
	{
		...
		if ( ChangeDisplaySettings( &dm, CDS_FULLSCREEN ) == DISP_CHANGE_SUCCESSFUL )
		{
			...
			*pheight = height;
			...
			gl_state.fullscreen = 1;

			ri.Con_Printf( PRINT_ALL, "ok\n" );

			if ( !VID_CreateWindow (width, height, 1) )
				return rserr_invalid_mode;
		...
		else
		{
			...
			*/
			if ( ChangeDisplaySettings( &dm, CDS_FULLSCREEN ) != DISP_CHANGE_SUCCESSFUL )
			{
				...
				*pwidth = width;
				*pheight = height;
				gl_state.fullscreen = 0;
				if ( !VID_CreateWindow (width, height, 0) )
					return rserr_invalid_mode;
...
void GLimp_Shutdown( void )
{
	...
	if (glw_state.hWnd)
	{
		...
		DestroyWindow (	glw_state.hWnd );		// Tomaz - leftover button bug fix
		glw_state.hWnd = NULL;
	}
	if ( glw_state.log_fp )
	{
...
#define OSR2_BUILD_NUMBER 1111
	...
	glw_state.allowdisplaydepthchange = 0;

	if ( GetVersionEx( &vinfo) )
	{
		if ( vinfo.dwMajorVersion > 4 )
		{
			glw_state.allowdisplaydepthchange = 1;
		}
...
int GLimp_InitGL (void)
{
    PIXELFORMATDESCRIPTOR pfd = 
	{
		...
		//32,								// 32-bit z-buffer	
		//0,								// no stencil buffer
		24,								// 24-bit z-buffer			// MrG - stencil shadows
		8,								// 8bit stencil buffer		// Stencil Shadows - MrG
	...
	*/
	if ( stereo->value != 0 )
	{
		...
		gl_state.stereo_enabled = 1;
	}
	else
	{
		gl_state.stereo_enabled = 0;
	}
	...
	*/
	if ( strstr( gl_driver->string, "opengl32" ) != 0 )
		glw_state.minidriver = 0;
	else
		glw_state.minidriver = 1;
	...
	*/
	...
	if ( glw_state.minidriver )
	{
		...
		if ( qwglSetPixelFormat( glw_state.hDC, pixelformat, &pfd) == 0 )
		{
	...
	{	
		// ChoosePixelFormat can give an error on some video drivers
		pixelformat = ChoosePixelFormat( glw_state.hDC, (const)&pfd);
		if ( pixelformat == 0 )
		{
			ri.Con_Printf (PRINT_ALL, "GLimp_Init() - ChoosePixelFormat failed\n");
			return 0;
		}
		if ( SetPixelFormat( glw_state.hDC, pixelformat, &pfd) == 0 )
		{
		...
		if ( !( pfd.dwFlags & PFD_GENERIC_ACCELERATED ) )
		{
			...
			if ( gl_allow_software->value )
				glw_state.mcd_accelerated = 1;
			else
				glw_state.mcd_accelerated = 0;
		}
		else
		{
			glw_state.mcd_accelerated = 1;
	...
	*/
	...
	/*if ( !VerifyDriver() )
	{
		...
		goto fail;
	}*/ // NeVo - allow GL to init on Microsoft Emulated GL Drivers
...
void UpdateGammaRamp()
{
	...
	if (gl_config.gammaramp) {
		...
		for (o=0; o<3; o++)
			for (i=0; i<256; i++) {
				signed int v;
				v = 255 * powf ( (i+0.5) * 0.0039138943248532289628180039138943, vid_gamma->value ) + 0.5;
				if (v > 255) v=255;
				if (v < 0) v=0;
...
** GLimp_AppActivate
*/
void GLimp_AppActivate( int active )
{
	if ( active )
	{
		SetForegroundWindow( glw_state.hWnd );
