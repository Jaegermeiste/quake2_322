...
{
	HRESULT ddrval;
	DDSURFACEDESC2 ddsd;	// NeVo
	DDSCAPS2 ddscaps;		// NeVo
	...
	// NeVo
	HRESULT (WINAPI *QDirectDrawCreate)( GUID FAR *lpGUID, LPVOID  *lplpDD, REFIID  iid, IUnknown FAR * pUnkOuter );

	ri.Con_Printf( PRINT_ALL, "Initializing DirectDraw 7\n");
	...
	*/
	...
	if ( ( QDirectDrawCreate = ( HRESULT (WINAPI *)( GUID FAR *, LPVOID  *, REFIID, IUnknown FAR * ) ) GetProcAddress( sww_state.hinstDDRAW, "DirectDrawCreateEx" ) ) == NULL )
	{
	...
	*/
	ri.Con_Printf( PRINT_ALL, "...creating DirectDraw 7 object: ");
#ifdef QDDRAWCOMPILERHACK
	myDDIID = &IID_IDirectDraw7;
	if ( ( ddrval = QDirectDrawCreate( NULL, &sww_state.lpDirectDraw, myDDIID, NULL ) ) != DD_OK )
#else
	if ( ( ddrval = QDirectDrawCreate( NULL, &sww_state.lpDirectDraw, IID_IDirectDraw7, NULL ) ) != DD_OK )
#endif
	{
...
#else
	if ((ddrval = sww_state.lpDirectDraw->SetCooperativeLevel (sww_state.hWnd,
		DDSCL_EXCLUSIVE|DDSCL_FULLSCREEN)) != DD_OK)
#endif
	{
		ri.Con_Printf( PRINT_ALL, "failed - %s\n",DDrawError (ddrval) );
...
#else
	if ((ddrval = sww_state.lpDirectDraw->SetDisplayMode (vid.width, vid.height, 8, 0, 0)) == DD_OK)
#endif
	{
		ri.Con_Printf( PRINT_ALL, "ok\n" );
	}
...
#else
	if ((ddrval = sww_state.lpDirectDraw->CreateSurface (&ddsd, &sww_state.lpddsFrontBuffer, NULL)) != DD_OK)
#endif
	{
		ri.Con_Printf( PRINT_ALL, "failed - %s\n", DDrawError( ddrval ) );
		goto fail;
...
#else
	if ((ddrval = sww_state.lpddsFrontBuffer->GetAttachedSurface (&ddsd.ddsCaps, &sww_state.lpddsBackBuffer)) != DD_OK)
#endif
	{
		ri.Con_Printf( PRINT_ALL, "failed - %s\n", DDrawError( ddrval ) );
		goto fail;
...
#else
	if ((ddrval = sww_state.lpDirectDraw->CreateSurface (&ddsd, &sww_state.lpddsOffScreenBuffer, NULL)) != DD_OK)
#endif
	{
		ri.Con_Printf( PRINT_ALL, "failed - %s\n", DDrawError( ddrval ) );
		goto fail;
	...
	*/
	...
	if ( ( ddrval = sww_state.lpDirectDraw->lpVtbl->CreatePalette( sww_state.lpDirectDraw,
														...
														NULL ) ) != DD_OK )
#else
	if ( ( ddrval = sww_state.lpDirectDraw->CreatePalette( DDPCAPS_8BIT | DDPCAPS_ALLOW256,
														palentries,
														&sww_state.lpddpPalette,
														NULL ) ) != DD_OK )
#endif
...
#else
	if ((ddrval = sww_state.lpddsFrontBuffer->SetPalette(sww_state.lpddpPalette))!=DD_OK)
#endif
	{
		ri.Con_Printf( PRINT_ALL, "failed - %s\n", DDrawError( ddrval ) );
		goto fail;
...
ri.Con_Printf( PRINT_ALL, "ok\n" );
	...
	*ppitch   = ddsd.lPitch;
	...
	return true;
fail:
	ri.Con_Printf( PRINT_ALL, "*** DirectDraw 7 init failure ***\n" );
...
void DDRAW_SetPalette( const unsigned char *pal )
{
	...
	if ( sww_state.lpddpPalette->lpVtbl->SetEntries( sww_state.lpddpPalette,
												...
												palentries ) != DD_OK )
#else
	if ( sww_state.lpddpPalette->SetEntries(    0,
												0,
												256,
												palentries ) != DD_OK )
#endif
...
void DDRAW_Shutdown( void )
{
	...
	if ( sww_state.lpDirectDraw )
	{
		...
		sww_state.lpDirectDraw->SetCooperativeLevel( sww_state.hWnd, DDSCL_NORMAL );
#endif
		ri.Con_Printf( PRINT_ALL, "...releasing DirectDraw 7\n");
#ifdef QDDRAWCOMPILERHACK
