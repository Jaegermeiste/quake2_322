...
cvar_t	*m_noaccel;		// Psychospaz - Mouse Acceleration Bug Fix
...
void IN_MLookDown (void) { mlooking = 1; }
...
int	mouseactive;	// 0 when not focus app
...
void IN_ActivateMouse (void)
{
	...
	if (m_noaccel->modified)	// Psychospaz - mouse acceleration bug fix
	{
		m_noaccel->modified = 0;
	}
	else
	{
		if (!in_mouse->value)
	...
	if(m_noaccel->value) 
		newmouseparms[2]=0;		// Psychospaz - mouse acceleration bug fix
	else 
		newmouseparms[2]=1;
...
// perform button actions
	for (i=0 ; i<mouse_buttons ; i++)
	{
		...
		{
			Key_Event (K_MOUSE1 + i, 1, sys_msg_time);
		}

		if ( !(mstate & (1<<i)) &&
			(mouse_oldbuttonstate & (1<<i)) )
		{
				Key_Event (K_MOUSE1 + i, 0, sys_msg_time);
		}
	}	
...
void IN_Init (void)
{
	...
	// Psychospaz - mouse acceleration bug fix
	m_noaccel				= Cvar_Get ("m_noaccel",				"0",		CVAR_ARCHIVE);
	m_autosens				= Cvar_Get ("m_autosens",				"1",		CVAR_ARCHIVE);
...
{ 
	int			numdevs;
	JOYCAPS		jc;
	MMRESULT	mmr = 0;		// NeVo - warning fix
	cvar_t		*cv;
...
void IN_Commands (void)
{
	...
	for (i=0 ; i < joy_numbuttons ; i++)
	{
		if ( (buttonstate & (1<<i)) && !(joy_oldbuttonstate & (1<<i)) )
		{
			...
			Key_Event (key_index + i, 1, 0);
		}

		if ( !(buttonstate & (1<<i)) && (joy_oldbuttonstate & (1<<i)) )
		{
			key_index = (i < 4) ? K_JOY1 : K_AUX1;
			Key_Event (key_index + i, 0, 0);
		}
	}
	...
	if (joy_haspov)
	{
		...
		for (i=0 ; i < 4 ; i++)
		{
			...
			{
				Key_Event (K_AUX29 + i, 1, 0);
			}

			if ( !(povstate & (1<<i)) && (joy_oldpovstate & (1<<i)) )
			{
				Key_Event (K_AUX29 + i, 0, 0);
			}
		}
