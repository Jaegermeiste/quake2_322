...
int GetScreenBufferLines (int *piLines);
int SetScreenBufferLines (int iLines);
int ReadText (LPTSTR pszText, int iBeginLine, int iEndLine);
int WriteText (LPCTSTR szText);
int CharToCode (char c);
int SetConsoleCXCY(HANDLE hStdout, int cx, int cy);
...
// so we'll know when to go away.
	heventDone = CreateEvent (NULL, 0, 0, NULL);
...
unsigned _stdcall RequestProc (void *arg)
{
	...
	while (1)
	{
		dwRet = WaitForMultipleObjects (2, heventWait, 0, INFINITE);
...
// height
	...
	if (cy < info.dwSize.Y)
	{
		if (!SetConsoleWindowInfo(hStdout, 1, &info.srWindow))
			return 0;
