...
void MoveClientToIntermission (edict_t *ent)
{
	...
	if (deathmatch->value || coop->value)
	{
		if (g_dmscoreboardmode->value == 0)
			DeathmatchScoreboardMessage (ent, NULL);
		else
			DeathmatchScoreboardMessage2 (ent, NULL);
...
void G_SetStats (edict_t *ent)
{
	...
	// timers
	//
	// Decker - display all active powerups
	...
	/*
	// NeVo - get rid of stupid center hand icon... we display the actual weapon now
	else if ( (ent->client->pers.hand == CENTER_HANDED || ent->client->ps.fov > 91)
		&& ent->client->pers.weapon)
		ent->client->ps.stats[STAT_HELPICON] = gi.imageindex (ent->client->pers.weapon->icon);
	*/
	else if ( (ent->client->ps.fov > cl_weapmaxfov->value) && ent->client->pers.weapon)	// NeVo
		ent->client->ps.stats[STAT_HELPICON] = gi.imageindex (ent->client->pers.weapon->icon);
	else
