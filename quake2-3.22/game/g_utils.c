...
*****************************/
//======================================================
// 1 if Ent is valid, has client, and edict_t inuse.
//======================================================
int G_EntExists(edict_t *ent) {
	return ((ent) && (ent->client) && (ent->inuse));
}

//======================================================
// 1 if ent is not DEAD or DEAD or DEAD (and BURIED!)
//======================================================
int G_ClientNotDead(edict_t *ent) {
	int buried=1;
	int b1=ent->client->ps.pmove.pm_type!=PM_DEAD;
	int b2=ent->deadflag != DEAD_DEAD;
	int b3=ent->health > 0;
	return ((b3 || b2 || b1) && buried);
}

//======================================================
// 1 if ent is not DEAD and not just did a Respawn.
//======================================================
int G_ClientInGame(edict_t *ent) {
	if (!G_EntExists(ent)) return 0;
	if (!G_ClientNotDead(ent)) return 0;
	return (ent->client->respawn_time + 5.0 < level.time);
}

//======================================================
// 1 if start and end are within radius distance.
//======================================================
int G_Within_Radius(vec3_t start, vec3_t end, float rad)
{
    vec3_t eorg = {0,0,0};
    int j;
...
int point_infront (edict_t *self, vec3_t point)
{
	vec3_t	vec;
	float	dot;
	vec3_t	forward;
