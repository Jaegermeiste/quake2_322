...
void GladiatorGun (edict_t *self)
{
	...
	{
		self->pos1[0] += crandom() * 640 * (FOG_CANSEEGOOD - r_fogdensity->value);
		self->pos1[1] += crandom() * 640 * (FOG_CANSEEGOOD - r_fogdensity->value);
		self->pos1[2] += crandom() * 320 * (FOG_CANSEEGOOD - r_fogdensity->value);
	}
...
// Lazarus
int gladiator_blocked (edict_t *self, float range)
{
