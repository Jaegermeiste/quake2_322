...
typedef struct gitem_s
{
	...
	int	(*pickup)(struct edict_s *ent, struct edict_s *other);
...
typedef struct monsterinfo_s
{
	...
	int	(*blocked)(edict_t *self, float dist);
...
//	void		(*abort_duck)(edict_t *self);
	...
	int	blindfire;		// will the monster blindfire?
...
//end Lazarus
	...
	// NeVo - locational damage for monsters
	int	lasthurt_location;
	int lasthurt_mod;
	int lasthurt_client;
} monsterinfo_t;
...
// NeVo - 3.22 cvars
extern	cvar_t	*g_diesink;
extern	cvar_t	*cl_weapmaxfov;		// Mmm... hacky
extern	cvar_t	*g_respawnprotect;
extern	cvar_t	*g_corpsegore;
extern	cvar_t	*g_autoplayerid;
extern	cvar_t	*g_grenadeshrapnel;
extern	cvar_t	*g_impulselog;
extern	cvar_t	*g_impulsekick;
extern	cvar_t	*g_rocketjump;
extern	cvar_t	*g_smartdamage;
extern	cvar_t	*g_dmscoreboardmode;
extern	cvar_t	*g_cooperativeexit;
extern	cvar_t	*r_fogdensity;
extern	cvar_t	*g_grenadehack;
...
void fire_blaster (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, int effect, int hyper);
...
void abortHeal (edict_t *ent,int mark);
...
void DeathmatchScoreboardMessage2 (edict_t *client, edict_t *killer);
...
// m_move.c
//
int M_CheckBottom (edict_t *ent);
int M_walkmove (edict_t *ent, float yaw, float dist);
...
typedef struct client_persistant_s
{
	...
	int         firemode; // Willi - for selecting fire modes
} client_persistant_t;
...
typedef struct client_respawn_s
{
	...
	int             firemode; // for selecting fire modes - Willi
} client_respawn_t;
...
struct gclient_s
{
	...
	int	showinventory;		// set layout stat
	int	showhelp;
	int	showhelpicon;
	...
	int			selectfire_count;	// Willi - machinegun fire modes
	int    backpedaling;  //<- CDawg added this
	...
	int			lasthurt_location;	// Where the client was hit		// Calrathan - locational damage

	float		respawn_time;		// can respawn when time > this

	edict_t		*chase_target;		// player we are chasing
	int	update_chase;		// need to update chase info?
};
...
/*****************************
Major Bitch's Helper Functions
*****************************/
extern int G_EntExists(edict_t *ent);
extern int G_ClientNotDead(edict_t *ent);
extern int G_ClientInGame(edict_t *ent);
extern int G_Within_Radius(vec3_t start, vec3_t end, float rad);
...
int parasite_drain_attack_ok (vec3_t start, vec3_t end);
...
int M_SetDeath(edict_t *self, mmove_t **deathmoves);
