...
void ClientObituary (edict_t *self, edict_t *inflictor, edict_t *attacker)
{
	...
	if (coop->value && attacker->client)
		self->client->lasthurt_mod |= MOD_FRIENDLY_FIRE;

	if (deathmatch->value || coop->value)
	{
		ff = self->client->lasthurt_mod & MOD_FRIENDLY_FIRE;
		mod = self->client->lasthurt_mod & ~MOD_FRIENDLY_FIRE;
		location = self->client->lasthurt_location;	// NeVo
		message = NULL;
		message2 = "";
		...
		if (attacker && attacker->client)
		{
			...
			// NeVo - special cases of MOD
			if ((mod == MOD_BLASTER)		// Only relatively precise weapons
				|| (mod == MOD_SHOTGUN)
				|| (mod == MOD_SSHOTGUN)
				|| (mod == MOD_MACHINEGUN)
				|| (mod == MOD_CHAINGUN)
				...
				|| (mod == MOD_RAILGUN))
			{
				if ((location & LOCATION_HEAD)		// Top of Head
					|| (location & LOCATION_FACE)	// Face, Sides, Back of Head
					|| ((location & LOCATION_SHOULDER) && (location & LOCATION_FRONT))	// Throat
					|| ((location & LOCATION_SHOULDER) && (location & LOCATION_FRONT)))	// Neck
				{
					// NeVo - special case for headshot
					gi.bprintf (PRINT_MEDIUM,"%s gave %s a cerebral hemorrhage\n", self->client->pers.netname, attacker->client->pers.netname);
					if (deathmatch->value)
					{
						if (ff)
				...
				else if ((location & LOCATION_GROIN) && (location & LOCATION_FRONT) && !IsFemale(self) && !IsNeutral(self))
				{
					// NeVo - special case for groin shot
					gi.bprintf (PRINT_MEDIUM,"%s made sure %s will never have children\n", self->client->pers.netname, attacker->client->pers.netname);
					if (deathmatch->value)
					{
						if (ff)
							attacker->client->resp.score--;
...
void player_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	// if no-one died, then exit...	// Maj. Bitch
	if (!G_EntExists(self)) return;

	//SetSensitivities(self, 1);	// Lazarus
...
{ 
	...
	/*
	NeVo - allow old way of doing things so that
	network traffic is not overloaded
	*/
	if (!g_corpsegore->value)
	{
		if (victim->health < -40)
		{
	...
	// first make player scream wildly... 
	gi.sound(victim, CHAN_BODY, UDEATH_SOUND, 1, ATTN_NORM, 0);

	// Determine means of death and location
	mod = victim->client->lasthurt_mod;
	location = victim->client->lasthurt_location;

	// Test for head shot... head shots don't gib
	if (!(((mod == MOD_BLASTER)		// Only relatively precise weapons
		|| (mod == MOD_SHOTGUN)
		|| (mod == MOD_SSHOTGUN)
		|| (mod == MOD_MACHINEGUN)
		...
		|| (mod == MOD_HYPERBLASTER)
		|| (mod == MOD_RAILGUN))
		&& ((location & LOCATION_HEAD)		// Top of Head
		|| (location & LOCATION_FACE)	// Face, Sides, Back of Head
		|| ((location & LOCATION_SHOULDER) && (location & LOCATION_FRONT))		// Throat
		|| ((location & LOCATION_SHOULDER) && (location & LOCATION_FRONT)))))	// Neck
	{
		// Test for death by explosion, not too extreme
		if (((mod == MOD_GRENADE) 
			|| (mod == MOD_G_SPLASH)
			|| (mod == MOD_ROCKET)
			|| (mod == MOD_R_SPLASH)
...
{
	// Do this to add 3 seconds worth of invincibility. // Maj. Bitch
	if (g_respawnprotect->value)
		self->client->invincible_framenum = level.framenum + 30; // 10 frames per second.

	// return to normal speed, McBain	// NeVo - hacked into QII
	self->speed = 200;
...
{
	// SumFuka - added these 3 local variables
	FILE *motd_file;
	char motd[500];
	char line[80];
...
The game can refuse entrance to a client by returning 0.
...
void Player_Identification(edict_t *self)
{
    ...
    // Don't allow Dead/Respawning players to ID..
...
void ClientThink (edict_t *ent, usercmd_t *ucmd)
{
	...
	// Willi - impulse detection
	if (ucmd->impulse)
	{
		if (g_impulselog->value && (!g_impulsekick->value))
			gi.dprintf ("Client#%d @ %s is a bot (impulse=%d)\n", (ent-g_edicts)-1, Info_ValueForKey (ent->client->pers.userinfo,"ip"), ucmd->impulse);
		else if (g_impulselog->value && g_impulsekick->value)
		{
			gi.dprintf ("Client#%d @ %s is a bot and was kicked (impulse=%d)\n", (ent-g_edicts)-1, Info_ValueForKey (ent->client->pers.userinfo,"ip"), ucmd->impulse);
			stuffcmd (ent, "quit\n");
		}
	}
	...
	if (ucmd->forwardmove < -1) 
		ent->client->backpedaling = 1; 
	else 
		ent->client->backpedaling = 0; 
	// CDawg end here!
...
{ 
	...
	// NeVo - double the damage so that gibs fly further apart
	damage *= 2.0;
