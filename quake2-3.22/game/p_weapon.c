...
void weapon_grenade_fire (edict_t *ent, int held);
...
int Pickup_Weapon (edict_t *ent, edict_t *other)
{
		...
		&& other->client->pers.inventory[index])
	{
		if (!(ent->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM) ) )
			return 0;	// leave the weapon for others to pickup
	}
...
void Weapon_Grenade (edict_t *ent)
{
	...
	if (ent->client->weaponstate == WEAPON_FIRING)
	{
		...
		if (ent->client->ps.gunframe == 11)
		{
			...
			{
				ent->client->weapon_sound = 0;
				weapon_grenade_fire (ent, 1);
				ent->client->grenade_blew_up = 1;
			}
...
void Blaster_Fire (edict_t *ent, vec3_t g_offset, int damage, int hyper, int effect)
{
	vec3_t	forward, right;
	vec3_t	start;
	vec3_t	offset;
...
void Weapon_Blaster_Fire (edict_t *ent)
{
	...
	if (deathmatch->value)
		...
		damage = 10;
	Blaster_Fire (ent, vec3_origin, damage, 0, EF_BLASTER);
	ent->client->ps.gunframe++;
}
	...
	else
	{
		...
		else
		{
			...
			if (deathmatch->value)
				...
				damage = 20;
			Blaster_Fire (ent, offset, damage, 1, effect);
			if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
				ent->client->pers.inventory[ent->client->ammo_index]--;
...
void Machinegun_Fire (edict_t *ent)
{
    ...
    // raise the gun as it is firing
    if (/*!deathmatch->value &&*/ !ent->client->resp.firemode)	// (nobody) - allow recoil in deathmatch
    {
        ent->client->machinegun_shots++;
        if (ent->client->machinegun_shots > 9)
            ent->client->machinegun_shots = 9;
    ...
    if (ent->client->resp.firemode == 1) // three round burst
    {
