...
void boss2_firebullet_right (edict_t *self)
{
	...
	if(r_fogdensity->value < FOG_CANSEEGOOD)
	{
		target[0] += crandom() * 640 * (FOG_CANSEEGOOD - r_fogdensity->value);
		target[1] += crandom() * 640 * (FOG_CANSEEGOOD - r_fogdensity->value);
		target[2] += crandom() * 320 * (FOG_CANSEEGOOD - r_fogdensity->value);
	}
...
int Boss2_CheckAttack (edict_t *self)
{
	vec3_t	spot1, spot2;
	vec3_t	temp;
	float	chance;
