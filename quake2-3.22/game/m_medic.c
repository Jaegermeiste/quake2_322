...
void cleanupHeal (edict_t *self, int change_frame)
{
...
void abortHeal (edict_t *self, int mark)
{
	edict_t	*temp;
...
// Lazarus: embedded returns 1 if argument entity's bounding box intersects
//          a solid.
int embedded (edict_t *ent)
{
	trace_t	tr;
...
//		return;
	...
	// If switch_paths is 1, we reached the end of a hint_chain. Just for grins,
...
void medic_fire_blaster (edict_t *self)
{
	...
	if(r_fogdensity->value < FOG_CANSEEGOOD)
	{
		end[0] += crandom() * 640 * (FOG_CANSEEGOOD - r_fogdensity->value);
		end[1] += crandom() * 640 * (FOG_CANSEEGOOD - r_fogdensity->value);
		end[2] += crandom() * 320 * (FOG_CANSEEGOOD - r_fogdensity->value);
	}
...
/*	vectoangles (dir, angles);
	...
	if (self->s.frame == FRAME_attack43)
	{
		...
		self->enemy->monsterinfo.aiflags |= AI_RESURRECTING;
		M_SetEffects(self->enemy);	// Lazarus
	}
...
// Lazarus
int medic_checkattack (edict_t *self)
{
	if (!(self->monsterinfo.aiflags & AI_MEDIC))
	{
		if( medic_FindDeadMonster(self) )
