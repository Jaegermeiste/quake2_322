...
Returns 1 if the inflictor can directly damage the target.  Used for
...
static int CheckArmor (edict_t *ent, vec3_t point, vec3_t normal, int damage, int te_sparks, int dflags)
{
	...
	// Make sure ent exists!	// Maj. Bitch
...
int CheckTeamDamage (edict_t *targ, edict_t *attacker)
{
...
/* 
============
G_LocationDamage - Calrathan
// NeVo - hacked this into Quake II
============
*/
int G_LocationDamage(vec3_t point, edict_t* targ, edict_t* attacker, int take) {
	vec3_t bulletPath;
	vec3_t bulletAngle;
	...
	int clientRotation;
	int bulletHeight;
	int bulletRotation;	// Degrees rotation around client. Used to check Back of Head vs. Face
	int impactRotation;
	...
	clientFeetZ  = targ->s.origin[2] + targ->mins[2]; //r.currentOrigin[2] + targ->r.mins[2];	
	// How tall the client is [Relative Z]
	clientHeight = targ->maxs[2] - targ->mins[2]; //r.maxs[2] - targ->r.mins[2];
	...
	VectorSubtract(targ->s.origin, point, bulletPath);
	...
	// The upper body never changes height, just distance from the feet
		...
		case LOCATION_LEG:
			take *= 0.7;
			if ((targ->health - take) < 100)	// NeVo - bug fix
				targ->speed -= take; // slowdown if shot in legs, McBain
			break;
		case LOCATION_FOOT:
			take *= 0.5;
			if ((targ->health - take) < 100)	// NeVo - bug fix
...
/*
NeVo - hacked version of G_LocationDamage for monsters, this could be done a lot better, but whatever
*/
int G_LocationDamageMonster(vec3_t point, edict_t* targ, edict_t* attacker, int take) {
	vec3_t bulletPath;
	vec3_t bulletAngle;
	...
	clientRotation = targ->s.angles[YAW]; //client->ps.viewangles[YAW];	// NeVo - I think this works
	bulletRotation = bulletAngle[YAW];
	...
	if (impactRotation < 90)
		targ->monsterinfo.lasthurt_location = LOCATION_BACK;
	else if (impactRotation < 180)
		targ->monsterinfo.lasthurt_location = LOCATION_RIGHT;
	else if (impactRotation < 270)
		targ->monsterinfo.lasthurt_location = LOCATION_FRONT;
	else if (impactRotation < 360)
		targ->monsterinfo.lasthurt_location = LOCATION_LEFT;
	else
		targ->monsterinfo.lasthurt_location = LOCATION_NONE;

	// The upper body never changes height, just distance from the feet
		if (bulletHeight > clientHeight - 2)
			targ->monsterinfo.lasthurt_location |= LOCATION_HEAD;
		else if (bulletHeight > clientHeight - 8)
			targ->monsterinfo.lasthurt_location |= LOCATION_FACE;
		else if (bulletHeight > clientHeight - 10)
			targ->monsterinfo.lasthurt_location |= LOCATION_SHOULDER;
		else if (bulletHeight > clientHeight - 16)
			targ->monsterinfo.lasthurt_location |= LOCATION_CHEST;
		else if (bulletHeight > clientHeight - 26)
			targ->monsterinfo.lasthurt_location |= LOCATION_STOMACH;
		else if (bulletHeight > clientHeight - 29)
			targ->monsterinfo.lasthurt_location |= LOCATION_GROIN;
		else if (bulletHeight < 4)
			targ->monsterinfo.lasthurt_location |= LOCATION_FOOT;
		else
			// The leg is the only thing that changes size when you duck,
			// so we check for every other parts RELATIVE location, and
			// whats left over must be the leg. 
			targ->monsterinfo.lasthurt_location |= LOCATION_LEG; 
		
		// Check the location ignoring the rotation info
		switch ( targ->monsterinfo.lasthurt_location & 
				~(LOCATION_BACK | LOCATION_LEFT | LOCATION_RIGHT | LOCATION_FRONT) )
		{
		case LOCATION_HEAD:
			take *= 1.8;
			break;
		case LOCATION_FACE:
			if (targ->monsterinfo.lasthurt_location & LOCATION_FRONT)
				take *= 5.0; // Faceshots REALLY suck
			else
				take *= 1.8;
			break;
		case LOCATION_SHOULDER:
			if (targ->monsterinfo.lasthurt_location & (LOCATION_FRONT | LOCATION_BACK))
				take *= 1.4; // Throat or nape of neck
			else
				take *= 1.1; // Shoulders
			break;
...
void T_Damage (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
{
	...
	// [L]arou[X] - rocket jumping
	if (((mod = MOD_R_SPLASH) && (targ == attacker)) && g_rocketjump->value)
    {
        damage *= 0.2; // Minor Damage	// NeVo
...
// figure momentum add
	...
	if (targ->client)
	{
		...
		// Modify the damage for location damage - Calrathan
		if (point && targ && targ->health > 0 && attacker && take && g_smartdamage->value)	// NeVo - cvar control
			take = G_LocationDamage(point, targ, attacker, take);
		else
			targ->client->lasthurt_location = LOCATION_NONE; 

	}
	// NeVo - do locational damage for monsters
	else if (targ->svflags & SVF_MONSTER)
	{
		// set the last client who damaged the target
		targ->monsterinfo.lasthurt_client = attacker->s.number;
		targ->monsterinfo.lasthurt_mod = mod;

		// Modify the damage for location damage - Calrathan
		if (point && targ && targ->health > 0 && attacker && take && g_smartdamage->value)	// NeVo - cvar control
			take = G_LocationDamageMonster(point, targ, attacker, take);
		else
			targ->monsterinfo.lasthurt_location = LOCATION_NONE;
	}
