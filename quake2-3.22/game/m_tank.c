...
//		gi.dprintf("shooting at feet!\n");
	...
	if(r_fogdensity->value < FOG_CANSEEGOOD)
	{
		vec[0] += crandom() * 640 * (FOG_CANSEEGOOD - r_fogdensity->value);
		vec[1] += crandom() * 640 * (FOG_CANSEEGOOD - r_fogdensity->value);
		vec[2] += crandom() * 320 * (FOG_CANSEEGOOD - r_fogdensity->value);
	}
...
void TankMachineGun (edict_t *self)
{
	...
	// check if enemy went away	// Lazarus
	if (!self->enemy || !self->enemy->inuse)
		return;
