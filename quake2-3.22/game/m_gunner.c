...
//       of the barrel is the same.	// Lazarus
...
// Lazarus
int gunner_grenade_check(edict_t *self)
{
	vec3_t		start;
...
void GunnerGrenade (edict_t *self)
{
	...
	AngleVectors (self->s.angles, forward, right, up);	// Lazarus
	G_ProjectSource (self->s.origin, monster_flash_offset[flash_number], forward, right, start);
	...
	if(skill->value < 2)
	{
		...
		dist = ((dist < 512 ) ? dist : 512);	//min(dist,512);	// NeVo - Intel compiler hack
