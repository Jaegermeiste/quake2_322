...
void SV_Baselines_f (void)
{
		...
		&& start < MAX_EDICTS)
	{
		...
		if (base->modelindex || base->sound || base->effects)
		{
			...
			MSG_WriteDeltaEntity (&nullstate, base, &sv_client->netchan.message, 1, 1);
		}
		start++;
	}
...
void SV_BeginDownload_f(void)
{
	...
	if (strstr (name, "..") || !allow_download->value
		...
		|| *name == '/'
		// next up, skin check
		// Vic - bug fix - these all had the wrong length values
...
void SV_ExecuteUserCommand (char *s)
{
	...
	Cmd_TokenizeString (s, 0);	// Redix - RCON_PASSWORD security bug fix (was 1)
