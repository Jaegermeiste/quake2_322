...
===============
*/
void PF_Unicast (edict_t *ent, int reliable)
{
	int		p;
	client_t	*client;
...
=================
*/
int PF_inPVS (vec3_t p1, vec3_t p2)
{
	int		leafnum;
	int		cluster;
	int		area1, area2;
...
=================
*/
int PF_inPHS (vec3_t p1, vec3_t p2)
{
	int		leafnum;
	int		cluster;
	int		area1, area2;
	...
	area2 = CM_LeafArea (leafnum);
	if ( mask && (!(mask[cluster>>3] & (1<<(cluster&7)) ) ) )
		return 0;		// more than one bounce away
	if (!CM_AreasConnected (area1, area2))
		return 0;		// a door blocks hearing
