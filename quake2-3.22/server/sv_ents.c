...
int SV_AddProjectileUpdate (edict_t *ent)
{
	if (!sv_projectiles)
...
void SV_EmitPacketEntities (client_frame_t *from, client_frame_t *to, sizebuf_t *msg)
{
	...
	while (newindex < to->num_entities || oldindex < from_num_entities)
	{
		...
		{	// delta update from old position
			// because the force parm is 0, this will not result
			// in any bytes being emited if the entity has not changed at all
			// note that players are always 'newentities', this updates their oldorigin always
			// and prevents warping
			MSG_WriteDeltaEntity (oldent, newent, msg, 0, newent->number <= maxclients->value);
			oldindex++;
			newindex++;
			continue;
		}
		...
		{	// this is a new entity, send it from the baseline
			MSG_WriteDeltaEntity (&sv.baselines[newnum], newent, msg, 1, 1);
			newindex++;
			continue;
		}
...
//*********** NiceAss Start ************
// fullCheck 1 means all points of the bounding box will be checked
// fullCheck 0 means only the origin of the entity will be checked
int SV_CheckPlayerVisible(vec3_t Angles, vec3_t start, edict_t *ent, int fullCheck, int predictEnt) {
	int		i;
	vec3_t	ends[9];
	vec3_t	entOrigin;
	trace_t	trace;
	...
	if ( !ent->client || sv_nc_visibilitycheck->value == 0 )
		return 1;
...
void SV_BuildClientFrame (client_t *client)
{
	...
	for (e=1 ; e<ge->num_edicts ; e++)
	{
		...
		visible = SV_CheckPlayerVisible(clent->client->ps.viewangles, start, ent, 1, 0);
		...
		if (!visible) {
			...
			visible = SV_CheckPlayerVisible(clent->client->ps.viewangles, start, ent, 0, 1);
		...
		*state = ent->s;
		...
		if (!visible && sv_nc_visibilitycheck->value == 1) {
			// The goal here is to make as much data 0 to the client.

			// Remove any model associations. Invisible.
			state->modelindex = state->modelindex2 = state->modelindex3 = state->modelindex4 = 0;
			// Lie about which way the player is facing.
			state->angles[0] = rand() % 360;		
			state->angles[1] = rand() % 360;		
			state->angles[2] = rand() % 360;		
...
void SV_RecordDemoMessage (void)
{
	...
	while (e < ge->num_edicts) 
	{
		...
		if (ent->inuse &&
			...
			MSG_WriteDeltaEntity (&nostate, &ent->s, &buf, 0, 1);
