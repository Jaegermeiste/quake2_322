...
==============
*/
void SV_WriteServerFile (int autosave)
{
	FILE	*f;
	cvar_t	*var;
...
void SV_GameMap_f (void)
{
	...
	{	// save the map just exited
		if (sv.state == ss_game)
		{
			...
			savedInuse = malloc(maxclients->value * sizeof(int));
			for (i=0,cl=svs.clients ; i<maxclients->value; i++,cl++)
			{
...
void SV_KillServer_f (void)
{
	...
	NET_Config ( 0 );	// close network sockets
}
