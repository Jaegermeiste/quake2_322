...
** Guy Paddock - now works with multitexture
*/
...
// recurse down the children, front side first
	...
	for ( c = node->numsurfaces, surf = r_worldmodel->surfaces + node->firstsurface; c ; c--, surf++)
	{
		...
		else
		{
			...
			if (qglMTexCoord2fSGIS)
			    R_DrawTriangleOutlines(surf);    // GuyP: gl_showtris fix
		}
	}
...
void R_DrawWorld (void)
{
	...
	*/
	...
	if (!r_skyboxfog->value)
	{
		// Echon - skybox fog bugfix
		qglDisable (GL_FOG);
		R_DrawSkyBox ();
	...
	if (!qglMTexCoord2fSGIS)	// GuyP: showtris under multitexture
