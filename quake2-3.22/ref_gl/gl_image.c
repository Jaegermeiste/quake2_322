...
int GL_Upload8 (byte *data, int width, int height,  int mipmap, int is_sky );
int GL_Upload32 (unsigned *data, int width, int height,  int mipmap);
...
void GL_EnableMultitexture( int enable )
{
...
void Scrap_Upload (void)
{
	...
	GL_Upload8 (scrap_texels[0], BLOCK_WIDTH, BLOCK_HEIGHT, 0, 0 );
	scrap_dirty = 0;
}
...
================
*/
// NeVo - Kempie ripped this form DarkPlaces for me
void GL_ResampleTextureLerpLine (byte *in, byte *out, int inwidth, int 
								 outwidth)
{
...
================
*/
void GL_LightScaleTexture (unsigned *in, int inwidth, int inheight, int only_gamma )
{
	if ( only_gamma )
	{
		int		i, c;
...
int GL_Upload32 (unsigned *data, int width, int height,  int mipmap)
{
	...
	if (mipmap) {
		...
		if (gl_config.glmipmap && gl_ext_mipmap->value) {
			qglTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP_SGIS, 1);	// Gage - SGIS GL Mipmapping
	...
	if (scaled && (scaled_width != width || scaled_height != height))
		free(scaled);
...
*/
/*
static int IsPowerOf2( int value )
{
	int i = 1;
