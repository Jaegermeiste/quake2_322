...
static void SCR_CalcVrect (void)
{
	...
	scr_vrect.x = (width - scr_vrect.width)/2;
...
void SCR_Init (void)
{
	...
	scr_drawall = Cvar_Get ("scr_drawall", "1", 0);		// NeVo - changed this

	// NeVo - 3.22 cvars
	cl_conheight = Cvar_Get ("cl_conheight", "0.5", CVAR_ARCHIVE);
	scr_blackclear = Cvar_Get ("scr_blackclear", "0", CVAR_ARCHIVE );
...
// decide on the height of the console
	if (cls.key_dest == key_console)
		scr_conlines = cl_conheight->value;		// half screen
	else
...
{
	S_StopAllSounds ();
	cl.sound_prepped = 0;		// don't play ambients
	CDAudio_Stop ();
...
void SCR_TileClear (void)
{
	...
	if ((scr_viewsize->value == 100) && ((int)camdevice.GetOption(camdevice.GetCurrentCam(), CAM_ASPECT) == ASPECT_NORMAL))	// NeVo - aspect ratio support
	...
	{	// clear above view screen
		...
		{
			// Draw black
			re.DrawFill(clear.x1 , clear.y1,
				clear.x2 - clear.x1 + 1, i - clear.y1+1, 0 );
		}
		clear.y1 = top;
	}
	if (clear.y2 > bottom)
	{	// clear below view screen
		...
		{
			// Draw black
			re.DrawFill (clear.x1, i,
				clear.x2-clear.x1+1, clear.y2-i+1, 0 );
		}
		clear.y2 = bottom;
	}
	if (clear.x1 < left)
	{	// clear left of view screen
		...
		else
		{
			// Draw black
			re.DrawFill (clear.x1, clear.y1,
				i-clear.x1+1, clear.y2 - clear.y1 + 1, 0 );
		}
		clear.x1 = left;
	}
	if (clear.x2 > right)
	{	// clear left of view screen
		...
		{
			// Draw black
			re.DrawFill (i, clear.y1,
				clear.x2-i+1, clear.y2 - clear.y1 + 1, 0 );
		}
		clear.x2 = right;
	}
...
NeVo - opened up the RTCW source and looked how they did it.
The influence shows. They said not to use sv.time because
it fluctuates to correct for network traffic and stuff, but
I don't think it does that in QII. I used Sys_Milliseconds.
==========
*/
void CL_DrawFPS( void ) {
	char		drawbuffer[24];
	static int	oldFPS[10];			// Don't lose the data
	int			i = 0, total = 0, len = 0;
	int			framerate;
	int			frametime;
	static int	oldtime;			// Don't lose the data
	static int	index;

	frametime = Sys_Milliseconds() - oldtime;
	oldtime = Sys_Milliseconds();
	
	// Shift and update the FPS array
	oldFPS[index % 10] = frametime;
	index++;

	// If no client active, return
	if ( cls.state != ca_active )
		return;
	...
	// Calculate Framerate
	framerate = 1000 * 10 / total;

	// Draw FPS
	Com_sprintf (drawbuffer, sizeof(drawbuffer), "%i FPS", framerate);
	len = strlen(drawbuffer);
	for (i=0; i<len; i++)
		re.DrawChar (viddef.width-4-len*FONTSIZE+i*FONTSIZE, FONTSIZE+1, 128 + drawbuffer[ i ] );
}
...
//				return;
		...
		else 
		{
			...
			//SCR_CalcVrect ();	// NeVo - this is obsolete
			...
			if(cl_drawFPS->value)	// Bram		// NeVo - made match RTCW
				CL_DrawFPS();
		}
	}
	re.EndFrame();
