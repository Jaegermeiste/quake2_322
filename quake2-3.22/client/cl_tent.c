...
void CL_ParseTEnt (void)
{
	...
	// PMM - fixed to correct for pitch of 0
		...
		ex->type = ex_misc;
		ex->ent.flags = RF_FULLBRIGHT|RF_TRANSLUCENT|RF_NOSHADOW;	// Aaron - RF_NOSHADOW
		ex->start = cl.frame.servertime - 100;
		ex->light = 150;
		ex->lightcolor[0] = 1;
		ex->lightcolor[1] = 1;
	...
	case TE_WELDING_SPARKS:
		...
		ex->ent.flags = RF_BEAM | RF_NOSHADOW;	// Aaron - RF_NOSHADOW
...
//				ent.origin[j] -= dist[j]*10.0;
		...
		while (d > 0)
		{
			...
			ent.model = b->model;
			ent.flags |= RF_NOSHADOW; // Valacar
			if (b->model == cl_mod_lightning)
			{
				ent.flags = RF_FULLBRIGHT | RF_NOSHADOW;	// NeVo
				ent.angles[0] = -pitch;
