...
//		--==OBSIDIAN UPDATE==--
//The variables declared here are used in M_Main_Draw to draw the alternate menu.
int ValuesSet = 0;
int useAltMenu = 1;
int base_height = 0;
int base_width = 0;
int circle_height = 0;
double base_angle = 0.0;	//This will be used to determine the angle of difference between
//each menu item in the alternate menu.
double start_angle = 0.0; //This is the angle at which to start drawing the alternate menu
double cur_angle = 0.0; //The angle at which to draw the current item.
double ToRad = 3.14158/180.0; //This is used to convert degrees to radians for the sin and cos functions.
int InTransit = 0;	//If the value is zero, then there is noa lockout on any key commands.
int FramesLeft = 0; //To determine whether the menu has finished moving into position.
double TransitAdjustAngle = 0.0;
...
int	m_entersound;		// play after drawing a frame, so caching
...
void M_Banner( char *name )
{
	...
	strcat(temp, "pics/alt_menus/");
	strcat(temp, name);

	//		--==OBSIDIAN UPDATE==--
	//An update for support of alternate menus
	if (useAltMenu == 1 && (FS_DoesFileExist(temp) == 1))
	{
		char temp2[MAX_QPATH];

		strcat(temp2, "alt_menus/");
		strcat(temp2, name);

		re.DrawGetPicSize (&w, &h, temp2 );
		re.DrawPic( viddef.width / 2 - w / 2, viddef.height / 2 - 110, temp2 );
	}
	else*/
	{
...
void M_PopMenu (void)
{
	...
	{
		M_ForceMenuOff ();
		MenuRefdefActive = 0;	// When menu forced off, allow the game refdef to be drawn again
	}
}
...
MAIN MENU
--==OBSIDIAN UPDATE==--
This function has been incredibly updated to support both the original menu
system and a new, alternate, round-robin menu system.
...
void M_Main_Draw (void)
{
	...
	int totalheight = 0;
	//		--==OBSIDIAN UPDATE==--
	//A check variable for use in determining alternate menus
	//int FirstCheck = 1;

	char litname[80];
	
	//		--==OBSIDIAN UPDATE==--
	//This variable will count the number of menu items.
	double NumMenuItems = 0.0;
	...
	// Draw rotating QII
	menuframework_t m_main_null; // Hack to not try to draw menu
	m_main_null.x = -1;
	m_main_null.y = -1;
	Menu_Draw (&m_main_null);

	// Draw menu on top
	//		--==OBSIDIAN UPDATE==--
	//Redoing this for loop to do some alternate menu checks for us!
	//useAltMenu = 1;
	if (ValuesSet == 0 && useAltMenu != 0){
		for (i = 0; names[i] != 0; i++){
			char Path[MAX_QPATH];
			Com_sprintf (Path, MAX_QPATH, "pics/alt_menus/%s.tga", names[i]);
			if (FS_DoesFileExist(Path) == 0){
				Com_sprintf (Path, MAX_QPATH, "pics/alt_menus/%s.pcx", names[i]);
				if (FS_DoesFileExist(Path) == 0){
					useAltMenu = 0;
					ValuesSet = 1;	//This is so we do not continuously do this check if we do not
									//have any alternate graphics.
				}
			}
		}
	}
	...
	//		--==OBSIDIAN UPDATE==--
	//I'm initing these variables here to help get rid of at least ONE of those damn warnings!
	w = 0;
	h = 0;

	//		--==OBSIDIAN UPDATE==--
	//This for loop has been reworked by me so many times, I don't know how different it is, but
	//I know it still does exactly what it was originally intended to do.
	for ( i = 0; names[i] != 0; i++ )
	{
		NumMenuItems++;
		...
		if (useAltMenu == 1){
			...
			Com_sprintf (Image, MAX_QPATH, "alt_menus/%s", names[i]);
			re.DrawGetPicSize(&w, &h, Image);
		}
		else{
		...
		if ( w > widest )
			widest = w;
		if ((double)h > base_height)
			base_height = (double)h;
		totalheight += ( h + 12 );
	}

	//		--==OBSIDIAN UPDATE==--
	//The main if statement defines the new alternate menu system. The else section draws the
	//original Quake 2 style main menu.
	if (useAltMenu == 1){
		if (ValuesSet == 0){
			circle_height = base_height * 1.5;
			base_angle = 360.0 / NumMenuItems;
			start_angle = 270.0;
			ValuesSet = 1;
		}

		if (InTransit == 1){
			start_angle += TransitAdjustAngle;
		...
		for (i = 0; names[i] != 0; i++){
			xoffset = (int)((double)circle_height * cos(cur_angle * ToRad));
			ystart = (int)((double)circle_height * sin(cur_angle * ToRad));

			xoffset += (viddef.width/2);
			ystart += (viddef.height/2);

			// - (base_height/2)
			if ( i != m_main_cursor ){
				char Image[MAX_QPATH];
				Com_sprintf (Image, MAX_QPATH, "alt_menus/%s", names[i]);
				re.DrawPic( xoffset - (base_height/2), ystart - (base_height/2), Image );
			}
			else{
				char Image[MAX_QPATH];
				Com_sprintf (Image, MAX_QPATH, "alt_menus/%s_sel", names[m_main_cursor]);
				re.DrawPic( xoffset - (base_height/2), ystart - (base_height/2), Image );
			}
...
const char *M_Main_Key (int key)
{
	...
	case K_DOWNARROW:
		if (InTransit == 0 ){
			...
			InTransit = 1;
			FramesLeft = 10;
			TransitAdjustAngle = -1.0 * (base_angle/(double)FramesLeft);
			return sound;
		}
	...
	case K_UPARROW:
		if (InTransit == 0){
			...
			InTransit = 1;
			FramesLeft = 10;
			TransitAdjustAngle = base_angle/(double)FramesLeft;
			return sound;
		}

	case K_KP_ENTER:
	case K_ENTER:
		if (InTransit == 1) break;
		m_entersound = 1;
...
static menuaction_t		s_options_soundmenu_action;	// NeVo
static menuaction_t		s_options_videomenu_action;	// NeVo
static menuaction_t		s_options_mainmenu_action;	// NeVo
static menuaction_t		s_player_setup_action;	// NeVo
...
void Options_MenuInit( void )
{
	...
	*/
	...
	s_options_customize_options_action.generic.y		= 0;
	s_options_customize_options_action.generic.name	= "Control Options";
	s_options_customize_options_action.generic.callback = CustomizeControlsFunc;
	
	s_options_videomenu_action.generic.type		= MTYPE_ACTION;
	s_options_videomenu_action.generic.x		= 0;
	s_options_videomenu_action.generic.y		= 10;
	s_options_videomenu_action.generic.name		= "Video Options";
	s_options_videomenu_action.generic.callback = M_Menu_Video_f;

	s_options_soundmenu_action.generic.type		= MTYPE_ACTION;
	s_options_soundmenu_action.generic.x		= 0;
	s_options_soundmenu_action.generic.y		= 20;
	s_options_soundmenu_action.generic.name		= "Sound Options";
	s_options_soundmenu_action.generic.callback = M_Menu_Sound_f;
...
*/
	...
	s_options_mainmenu_action.generic.type		= MTYPE_ACTION;
	s_options_mainmenu_action.generic.x			= 0;
	s_options_mainmenu_action.generic.y			= 150;
	s_options_mainmenu_action.generic.name		= "Main Menu";
	s_options_mainmenu_action.generic.callback	= M_PopMenu;

	ControlsSetMenuItemValues();

	Menu_AddItem( &s_options_menu, ( void * ) &s_options_customize_options_action );
	Menu_AddItem( &s_options_menu, ( void * ) &s_options_videomenu_action );
	Menu_AddItem( &s_options_menu, ( void * ) &s_options_soundmenu_action );
	Menu_AddItem( &s_options_menu, ( void * ) &s_player_setup_action );			// NeVo
	...
	Menu_AddItem( &s_options_menu, ( void * ) &s_options_mainmenu_action );
}
...
//PGM
	...
	{
		Cvar_SetValue ("deathmatch", 1 );	// deathmatch is always 1 for rogue games, right?
...
void StartServer_MenuDraw(void)
{
	...
	M_Banner ("m_banner_start_server");			// NeVo - found this buried in the pak
}
...
{ 
   ...
   // Knightmare- only register model and skin on starup and when changed 
...
// Reworked by Obsidian
static int IconOfSkinExists( char *skin, char **pcxfiles, int npcxfiles )
{
	int i;
	char BASEscratch[1024];
	char EXMNscratch[1024];

	memset(EXMNscratch, 0, 1024);
	strcpy( EXMNscratch, skin );
	*strrchr( EXMNscratch, '.' ) = 0;
	strcat( EXMNscratch, "_i" );

	for ( i = 0; i < npcxfiles; i++ )
	{
		memset(BASEscratch, 0, 1024);
		strcpy(BASEscratch, pcxfiles[i]);
		*strrchr(BASEscratch, '.') = 0;
		if ( strcmp( BASEscratch, EXMNscratch ) == 0 )
			return 1;
	}
	...
	do
	{
		...
		*/
		...
		for ( i = 0; i < npms; i++ )
		{
			...
			//		--==OBSIDIAN UPDATE==--
			//Changing the if statement to suit the search through pack files
			if (!FS_DoesFileExist(scratch))
			{
				free( dirnames[i] );
				dirnames[i] = 0;
				continue;
			...
			//		--==OBSIDIAN UPDATE==--
			//Skin Loading enhancement! adding support for .tga files (at least when it comes
			...
			{	
				//		--==OBSIDIAN UPDATE==--
				//Just so MD2s can have TGA support as well
				strcpy( scratch, dirnames[i] );
				strcat( scratch, "/*.tga" );
				pcxnames = FS_ListFiles( scratch, &npcxfiles, 0, SFF_SUBDIR | SFF_HIDDEN | SFF_SYSTEM );
				if (!pcxnames){
		...
		// Knightmare- if no valid player models found in path, 
...
int PlayerConfig_MenuInit( void )
{
	extern cvar_t *name;
	extern cvar_t *team;
	extern cvar_t *skin;
...
void PlayerConfig_MenuDraw( void )
{
	...
	refdef.lightstyles = 63;	// FULLBRIGHT
	...
	if ( s_pmi[s_player_model_box.curvalue].skindisplaynames )
	{
		...
		//entity.weapon = weaponmodel;
		player.flags = RF_FULLBRIGHT;
		player.origin[0] = 80;
		player.origin[1] = 0;
		player.origin[2] = 0;
		...
		player.oldframe = 0;
		player.backlerp = 0.0;
		player.angles[1] = yaw + 0.57;
		if ( ++yaw > 360 )
			yaw -= 360;

		/*
		// NeVo - hack in weapon model display
		memset( &weapon, 0, sizeof( weapon ) );
		weapon.model = weaponmodel;
		weapon.flags = RF_FULLBRIGHT;
		weapon.origin[0] = player.origin[0] + (3 * cosf(player.angles[1]));	// X
		weapon.origin[1] = player.origin[1] + (3 * sinf(player.angles[1]));	// Y
		weapon.origin[2] = player.origin[2] + 5;							// Z
		VectorCopy( weapon.origin, weapon.oldorigin );
		weapon.frame = 0;
		weapon.oldframe = 0;
		weapon.backlerp = 0.0;
		weapon.angles[1] = atanf(weapon.origin[1]/weapon.origin[0]);
		if ( ++yaw > 360 )
			yaw -= 360;
...
void M_Init (void)
{
	...
	Cmd_AddCommand ("menu_options", M_Menu_Options_f);
		Cmd_AddCommand ("menu_sound", M_Menu_Sound_f );	// NeVo
