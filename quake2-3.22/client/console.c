...
// draw the background
	...
	//[QBS]  Time Tutorial
	Com_sprintf (version, sizeof(version), "%s %4.2f%s.%i", GAMENAME, VERSION, BUILD);//NeVo - modified
	len = strlen(version);
	for (x=0 ; x<len ; x++)
		re.DrawChar (viddef.width-4-len*FONTSIZE+x*FONTSIZE, lines-25, 128 + version[x] );// DMP adjust by size
	...
	len = strlen(tmpbuf);
	for (x=0 ; x<len ; x++)
		re.DrawChar (viddef.width-4-len*FONTSIZE+x*FONTSIZE, lines-15, 128 + tmpbuf[x] );
	//[QBS]end
...
// draw the text
	...
	y = lines - 35;				// NeVo - adjusted this to not interfere with the version display
#endif
