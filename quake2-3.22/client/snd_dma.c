...
cvar_t		*s_mixahead;
cvar_t		*s_primary;
cvar_t		*s_musicvolume;	// Vic - stereo sound
cvar_t      *s_swapstereo;	// Vic - stereo sound
cvar_t		*s_balance;		// NeVo - sound balance control
...
void S_Init (void)
{
	...
	else
	{
		...
		s_musicvolume = Cvar_Get ("s_musicvolume", "0.25", CVAR_ARCHIVE);	// Vic - stereo sound
		s_swapstereo = Cvar_Get ("s_swapstereo", "0", CVAR_ARCHIVE);	// Vic - stereo sound
		s_balance = Cvar_Get ("s_balance", "5", CVAR_ARCHIVE);	// NeVo - sound balance
		...
		S_InitScaletable ();
		S_InitMusicScaletable ();	// Vic - stereo sound
...
==================
*/
sfx_t *S_FindName (char *name, int create)
{
	int		i;
	sfx_t	*sfx;
...
void S_SpatializeOrigin (vec3_t origin, float master_vol, float dist_mult, int *left_vol, int *right_vol)
{
	...
	{
		// NeVo - modified for stereo balance
		*left_vol = 255 * ((10.0 - s_balance->value) * 0.1);
		*right_vol = 255 * (s_balance->value * 0.1);
		return;
	}
...
// calculate stereo seperation and distance attenuation
	...
	{ // no attenuation = no spatialization
		// NeVo - modified for balance control
		lscale = 1.0;
		rscale = 1.0;
	}
	else
	...
	*right_vol = (int) (master_vol * scale) * (s_balance->value * 0.1);
	if (*right_vol < 0)
		*right_vol = 0;

	scale = (1.0 - dist) * lscale;
	*left_vol = (int) (master_vol * scale) * ((10.0 - s_balance->value) * 0.1);
	if (*left_vol < 0)
		*left_vol = 0;
}
...
void S_Spatialize(channel_t *ch)
{
	...
	{
		// NeVo - modified for stereo balance
		ch->leftvol = ch->master_vol * ((10.0 - s_balance->value) * 0.1);
		ch->rightvol = ch->master_vol * (s_balance->value * 0.1);
		return;
	}
...
struct sfx_s *S_RegisterSexedSound (entity_state_t *ent, char *base)
{
	...
	sfx = S_FindName (sexedFilename, 0);
...
void S_AddLoopSounds (void)
{
	...
	for (i=0 ; i<cl.frame.num_entities ; i++)
	{
		...
		ch->autosound = 1;	// remove next frame
		ch->sfx = sfx;
                /*
                 * PATCH: eliasm
                 *
...
// mix some sound
	...
	// NeVo - run the winamp per-frame code
	S_WinAmp_Frame();
}
