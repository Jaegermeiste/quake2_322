...
void CL_ParseFrame (void)
{
	...
	{
		cl.frame.valid = 1;		// uncompressed frame
		old = NULL;
		cls.demowaiting = 0;	// we can start recording now
	}
	else
	{
		...
		else if (cl.parse_entities - old->parse_entities > MAX_PARSE_ENTITIES-128)
		{
			...
			cl.frame.valid = 1;	// valid delta parse
	}
	...
	if (cl.frame.valid)
	{
		...
		cl.sound_prepped = 1;	// can start mixing ambient sounds
...
void CL_AddPacketEntities (frame_t *frame)
{
	...
	float				bounce;				// Pooy - QIII style bobbing pickups

	// bonus items rotate at a fixed rate
	autorotate = anglemod(cl.time/10);						
	bounce = ( sin( autorotate * M_PI * 2 / 360 ) * 7 );	// Pooy - bobbing pickups

	// brush models can auto animate their frames
	autoanim = 2*cl.time/1000;

	memset (&ent, 0, sizeof(ent));
	VectorCopy(vec3_origin, clientOrg);	// NeVo
...
//============
		...
		if ( effects & EF_BLASTER
			|| effects & EF_ROCKET
			|| effects & EF_HYPERBLASTER
			|| effects & EF_FLIES
			|| effects & EF_TELEPORTER) 
			ent.flags |= RF_NOSHADOW;	// Aaron/Kong
		...
		{	
			...
			// Skip this if we dont wan't to rotate/bob everything
			if (effects & EF_ROTATE_ZHACKUP)
			{
				if (cl_itemboball->value)
					ent.angles[1] = autorotate;
			}
			else
				ent.angles[1] = autorotate;

			// All items fit inside this, theoretically
			VectorSet ( clipmins, -16, -16, -16 );
			VectorSet ( clipmaxs, 16, 16, 16 );

			// Set the mask to clip to almost everything
			this_mask = (CONTENTS_SOLID
				|CONTENTS_PLAYERCLIP
			...
			if ( effects & EF_ROTATE_ZHACKUP )
			{
				/*
				NeVo - 16 Jan, 2003
				This is a colossal hack overloading EF_SPINNINGLIGHTS
				since something is either rotating OR a spinning light.
				If the entity has this flag we boost its origin z.
				*/
				if (cl_itemboball->value && cl_itembob->value)
				{
					vec3_t temporg;
					VectorCopy (ent.origin, temporg);
					temporg[2] -= 5;	// Set the z negative to what should be below the floor
					tr = CM_TransformedBoxTrace (ent.origin, temporg, clipmins, clipmaxs, 0, this_mask, ent.origin, ent.angles);
					ent.origin[2] = 16 + tr.endpos[2];
				}
			}
			...
			if ( cl_itembob->value )
			{
				...
				{
					if (cl_itemboball->value)
						ent.origin[2] += bounce;		// Pooy - bouncing items
				}
				else if (effects & EF_ROTATE_ZHACKDOWN)
				{
					if (cl_itemboball->value)
						ent.origin[2] += bounce;		// Pooy - bouncing items
				}
			...
			if ( effects & EF_ROTATE_ZHACKDOWN )
			{
				/*
				NeVo - 16 Jan, 2003
				This is a colossal hack overloading EF_TRACKERTRAIL
				since id set it up to be safe to use it this way
				If the entity has this flag we lower its origin z.
				This is primarily a hack for powerups in tight spaces,
				like the silencer under the stairs in the first level.
				We also add a light, since its so hard to see in there.
				*/
				ent.origin[2] -= 2;		// NeVo - fix the silencer being in the concrete in the original game
				if (cl_itemboball->value && cl_itembob->value)
				{
					ent.origin[2] -= 9;
					V_AddLight (ent.origin, 85, -1.0, -1.0, -1.0);
				}
			}
			...
			{
				// Clip the origin
				tr = CM_TransformedBoxTrace (ent.origin, ent.origin, clipmins, clipmaxs, 0, this_mask, ent.origin, ent.angles);
				VectorCopy ( tr.endpos, ent.origin );
			}
			if (cl_itemboball->value || cl_itembob->value)
				VectorCopy ( ent.origin, ent.oldorigin );		// This smooths movement greatly
		}
		// RAFAEL
...
//pmm
		// Pooy - fullbright regardless of x_modulate
		if (cl_fullbright->value)
...
void CL_AddViewWeapon (player_state_t *ps, player_state_t *ops)
{
	...
	if(hand->value == 2.0f)
	{
		vec3_t right, up;
		// Get the movement
		AngleVectors(gun.angles, NULL, right, up);
		// Move the gun
		VectorScale(right, -8.0f, right);
		VectorScale(up, -5.0f, up);
		VectorAdd(gun.origin, right, gun.origin);
		VectorAdd(gun.origin, up, gun.origin);
	}
...
void CL_CalcViewValues (void)
{
	...
	if ( Q_fabs(ops->pmove.origin[0] - ps->pmove.origin[0]) > 256*8
		|| Q_fabs(ops->pmove.origin[1] - ps->pmove.origin[1]) > 256*8
		|| Q_fabs(ops->pmove.origin[2] - ps->pmove.origin[2]) > 256*8)
