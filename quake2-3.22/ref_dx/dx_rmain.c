...
cvar_t	*r_antialias;
cvar_t	*r_antialiaslevel;

/*
DX_DrawQuadTex - NeVo
*/
void DX_DrawQuadTexScreen (float x, float y, float w, float h, float sl, float sh, float tl, float th)
{
	QVERTEX	*vertices;

	memset(vertices, 0, sizeof(QVERTEX)*4);
	...
	d3dvb->Lock(0, sizeof(vertices), vertices, NULL);

	d3dDev9->DrawPrimitive( D3DPT_TRIANGLEFAN, 0, 2 );
}
...
void R_RenderView (refdef_t *fd)
{
	...
	if (r_waterfog->value && (r_newrefdef.rdflags & RDF_UNDERWATER))
	{
		...
		if (gl_ext_fog_coord->value && gl_config.coordinatefog)
			waterfog.usecoords = true;
		else
			waterfog.usecoords = false;
	...
	else if (r_skyfog->value)
	{
		...
		if (gl_ext_fog_coord->value && gl_config.coordinatefog)
			skyfog.usecoords = true;
		else
			skyfog.usecoords = false;
...
void R_Register( void )
{
	...
	gl_clear = ri.Cvar_Get ("gl_clear", "0", CVAR_ARCHIVE);			// NeVo - made this save
	...
	r_fluidwaves = ri.Cvar_Get( "r_fluidwaves", "1.85", CVAR_ARCHIVE);
	r_drawbboxes = ri.Cvar_Get( "r_drawbboxes", "0", CVAR_ARCHIVE);
	r_skyboxsize = ri.Cvar_Get( "r_skyboxsize", "2300", CVAR_ARCHIVE);
	r_skyfog = ri.Cvar_Get( "r_skyfog", "0", CVAR_ARCHIVE );	// NeVo - sky fog would prob be fine if it was a skydome
	r_waterfog = ri.Cvar_Get( "r_waterfog", "1", CVAR_ARCHIVE );
	r_skyboxfog = ri.Cvar_Get( "r_skyboxfog", "1", CVAR_ARCHIVE );
	...
	gl_ext_fog_coord = ri.Cvar_Get( "gl_ext_fog_coord", "1", CVAR_ARCHIVE);
	...
	r_antialias = ri.Cvar_Get( "r_antialias", "0", CVAR_ARCHIVE);
	r_antialiaslevel = ri.Cvar_Get( "r_antialiaslevel", "16", CVAR_ARCHIVE);
...
qboolean R_SetMode (void)
{
	...
	r_skyboxsize->modified = true;		// DMP skybox size change
	...
	*/
	...
	if ( strstr( gl_config.extensions_string, "GL_EXT_fog_coord" ) )
	{
		if(!gl_ext_fog_coord->value)
		{
			...
			gl_config.coordinatefog = false;
		}
		else
		{
			ri.Con_Printf(PRINT_ALL, "...using GL_EXT_fog_coord\n");
			gl_config.coordinatefog = true;
		}
	}
	else
	{
	...
	if ( strstr( gl_config.extensions_string, "GL_ARB_texture_compression" ) )
	{
		if(!gl_ext_texture_compression->value)
		{
			...
			gl_config.texture_compression = false;
		}
		else
		{
			ri.Con_Printf(PRINT_ALL, "...using GL_ARB_texture_compression\n");
			gl_config.texture_compression = true;
		}
	}
	else
	{
