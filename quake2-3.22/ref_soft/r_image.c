...
image_t *GL_LoadPic (char *name, byte *pic, int width, int height, imagetype_t type, int bits)
{
	...
	int			i, c, b;
	//		--==OBSIDIAN UPDATE==--
	//These are used for better TGA support in software mode
	//based on work by HTrigger
	...
	//		--==OBSIDIAN UPDATE==--
	//TGA file support in software
	//based on work by HTrigger
	if (type == it_wall || bits == 32){
		...
		if (bits == 32){
			...
			GL_BuildPalettedTexture(paletted_texture, ( unsigned char * ) pic, 
...
image_t	*R_FindImage (char *name, imagetype_t type)
{
	...
	for (i=0, image=r_images ; i<numr_images ; i++,image++)
	{
		...
		strcat(PCXName, ".pcx");
		strcat(WALName, ".wal");
		if (!strcmp(TGAName, image->name) || !strcmp(PCXName, image->name) || !strcmp(WALName, image->name))
		{
			image->registration_sequence = registration_sequence;
			return image;	
		}
