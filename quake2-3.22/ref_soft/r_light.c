...
//PGM
	/*
	NeVo - for some idiot reason PGM was using int i here... changed to a float
	*/
...
//=====
	...
	// NeVo - added dlight cutoff
	if (dist > (temp - r_dlightcutoff->value))	// PGM (dist > light->intensity)
	{
...
// mark the polygons
	...
	for (i=0 ; i<node->numsurfaces ; i++, surf++)
	{
		...
		surf->dlightbits |= bit;
		/*
		NeVo - this is how it is in GL... is this better?
		if (surf->dlightframe != r_dlightframecount)
		{
			surf->dlightbits = bit;
			surf->dlightframe = r_dlightframecount;
...
//=====
		...
		minlight = r_dlightcutoff->value;		// NeVo - made configurable
		if (rad < minlight)
			continue;
...
//PGM
				if(!negativeLight)
				{
					if (dist < minlight)
						...
						blocklights[t*smax + s] += (minlight - dist) * 256; // Riot - better dlight falloff
				}
				else
				{
					if (dist < minlight)
