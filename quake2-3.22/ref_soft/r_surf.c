...
// colect and free surfcache_t blocks until the rover block is large enough
	...
	// free another
		...
		qnew->size += sc_rover->size;
		qnew->next = sc_rover->next;
	}

// create a fragment out of any leftovers
	if (qnew->size - size > 256)
	{
		sc_rover = (surfcache_t *)( (byte *)qnew + size);
		sc_rover->size = qnew->size - size;
		sc_rover->next = qnew->next;
		sc_rover->width = 0;
		sc_rover->owner = NULL;
		qnew->next = sc_rover;
		qnew->size = size;
...
// DEBUG
	if (width > 0)
		qnew->height = (size - sizeof(*qnew) + sizeof(qnew->data)) / width;

	qnew->owner = NULL;              // should be set properly after return
