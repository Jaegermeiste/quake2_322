...
// current viewleaf
	...
	/*else {	// Obsidian was too dumb to comment this - NeVo
		Draw_StretchPic(0, 0, vid.width, vid.height, "q2bg.tga");
	}*/

	// NeVo - sad hack to prevent water warp when in cinematic mode >:-(
	if (sw_waterwarp->value && (r_newrefdef.rdflags & RDF_UNDERWATER) && (!vid_widescreen->value))
		r_dowarp = true;
	else
		r_dowarp = false;
