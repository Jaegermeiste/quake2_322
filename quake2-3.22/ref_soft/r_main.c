...
//PGM
	...
	// NeVo - 3.22 cvars
	r_dlightcutoff = ri.Cvar_Get ("r_dlightcutoff", "0", CVAR_ARCHIVE);
	r_crashhack = ri.Cvar_Get ("r_crashhack", "0", CVAR_ARCHIVE);
	vid_widescreen = ri.Cvar_Get ("vid_widescreen", "0", CVAR_ARCHIVE);
}
...
void Draw_BuildGammaTable (void)
{
	...
	for (i=0 ; i<256 ; i++)
	{
		inf = 255 * (int)powf ( (float)(i+0.5)/255.5 , (float)g ) + 0.5;
		if (inf < 0)
			inf = 0;
		if (inf > 255)
			inf = 255;
