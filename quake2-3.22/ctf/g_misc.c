...
#define FADE_SCALE		0.05		// NeVo - gib, head, and debris fade
#define SINK_DELTA		1.9			// NeVo - gib, head, and debris sink into floor
...
{
	if ((self->s.modelindex == sm_meat_index) && (self->s.frame < 10))
		self->s.frame++;

	self->nextthink = level.time + FRAMETIME;
	
	// NeVo - gib fade
	self->s.renderfx |= RF_TRANSLUCENT;		// NeVo - gib fade
	self->s.effects |= EF_SPHERETRANS;
	VectorScale(self->size, FADE_SCALE, self->size);	// NeVo - shrink gib into oblivion
	self->s.origin[2] -= SINK_DELTA;					// NeVo - sink gib into floor
	if ( VectorLength(self->size) < 0.1 )		// NeVo - shrink gib into oblivion
	{
		self->think = gib_die;
	}
}
...
{	
	...
	// NeVo - head fade
	self->s.renderfx |= RF_TRANSLUCENT;		// NeVo - head fade
	self->s.effects |= EF_SPHERETRANS;
	VectorScale(self->size, FADE_SCALE, self->size);	// NeVo - shrink head into oblivion
	self->s.origin[2] -= SINK_DELTA;					// NeVo - sink head into floor
	if ( VectorLength(self->size) < 0.1 )		// NeVo - shrink head into oblivion
	{
		self->think = gib_die;
	}
}
...
// NeVo - debris fade
void debris_think (edict_t *self)
{
	self->nextthink = level.time + FRAMETIME;
	
	// NeVo - debris fade
	self->s.renderfx |= RF_TRANSLUCENT;		// NeVo - debris fade
	self->s.effects |= EF_SPHERETRANS;
	VectorScale(self->size, FADE_SCALE, self->size);	// NeVo - shrink debris into oblivion
	self->s.origin[2] -= SINK_DELTA;					// NeVo - sink debris into floor
	if ( VectorLength(self->size) < 0.1 )		// NeVo - shrink debris into oblivion
	{
		self->think = debris_die;
	}
}

// NeVo - ripped off gib touch function
void debris_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	vec3_t	normal_angles, right;
...
void ThrowDebris (edict_t *self, char *modelname, float speed, vec3_t origin)
{
	...
	chunk->think = debris_think;
	chunk->nextthink = level.time + 5 + random()*5;
	chunk->s.frame = 0;
	chunk->flags = 0;
	chunk->classname = "debris";
	chunk->takedamage = DAMAGE_YES;
	chunk->touch = debris_touch;	// NeVo - debris fade
...
void SP_misc_gib_arm (edict_t *ent)
{
	...
	ent->think = head_think;				// NeVo - corpse fade
...
void SP_misc_gib_head (edict_t *ent)
{
	...
	ent->avelocity[1] = random()*200;
	ent->avelocity[2] = random()*200;
	ent->think = head_think;				// NeVo - head fade
	ent->nextthink = level.time + 30;
	gi.linkentity (ent);
}
