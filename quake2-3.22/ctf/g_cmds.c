...
void Cmd_Use_f (edict_t *ent)
{
	...
	else if (!Q_strcasecmp(s, ent->client->pers.weapon->pickup_name))
    {
        if (!Q_strcasecmp(s, "machinegun"))
        {
            if (ent->client->resp.firemode == 0)
            {
				...
				gi.cprintf(ent, PRINT_HIGH, "Fire Selector: 3 Round Burst\n");
            }
            else if (ent->client->resp.firemode == 1)
            {
                ent->client->resp.firemode = 2;
                gi.cprintf(ent, PRINT_HIGH, "Fire Selector: Semi-Automatic\n");
            }
            else
            {
                ent->client->selectfire_count=0;
                ent->client->resp.firemode = 0;
                gi.cprintf(ent, PRINT_HIGH, "Fire Selector: Full Automatic\n");
            }
        }
    }
    ...
    {
        ent->client->resp.firemode = 0;
    }*/	// NeVo - resetting the fire mode to automatic is annoying as hell
...
void Cmd_InvDrop_f (edict_t *ent)
{
	...
	// Not available to dead or respawning players! // Maj. Bitch
