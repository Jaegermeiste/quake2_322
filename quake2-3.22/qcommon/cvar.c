...
============
*/
static int Cvar_InfoValidate (char *s)
{
	if (strstr (s, "\\"))
		return 0;
	if (strstr (s, "\""))
...
{
	return Cvar_Set2 (var_name, value, 1);
}
...
{
	return Cvar_Set2 (var_name, value, 0);
}
...
{
	if (Cvar_VariableValue(Cmd_Argv(1)) == 1)
		Cvar_Set (Cmd_Argv(1), "0");
	else if (Cvar_VariableValue(Cmd_Argv(1)) == 0)
...
void Cvar_Init (void)
{
	...
	Cmd_AddCommand ("toggle", Cvar_Toggle_f);	// NeVo - cvar toggle command
