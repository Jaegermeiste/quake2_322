...
int	portalopen[MAX_MAP_AREAPORTALS];
...
cmodel_t *CM_LoadMap (char *name, int clientload, unsigned *checksum)
{
	...
	strcpy(mapname, "exec ");
	if (cl_execmappath->string[0])
		strcat(mapname, cl_execmappath->string);
	strcat(mapname, name + 5);		// skip "/maps"
	mapname[strlen(mapname)-4] = 0;	// remove ".bsp"
	strcat(mapname, ".cfg\n");
	if (cl_execmapcfg->value)
		Cbuf_AddText (mapname);
	// Pooy - autmatic mapscript execution
...
trace_t	trace_trace;
int		trace_contents;
int	trace_ispoint;		// optimized case
					  ...
					  trace_t *trace, cbrush_t *brush)
{
	...
	for (i=0 ; i<brush->numsides ; i++)
	{
		...
		if (d2 > 0)
			getout = 1;	// endpoint is not in solid
		if (d1 > 0)
			startout = 1;
					  ...
					  trace_t *trace, cbrush_t *brush)
{
	...
	// inside this brush
	trace->startsolid = trace->allsolid = 1;
	trace->fraction = 0;
	trace->contents = brush->contents;
}
...
Returns 1 if any leaf under headnode has a cluster that
is potentially visible
=============
*/
int CM_HeadnodeVisible (int nodenum, byte *visbits)
{
	int		leafnum;
	int		cluster;
	cnode_t	*node;
