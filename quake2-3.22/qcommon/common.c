...
void Com_Printf (char *fmt, ...)
{
	...
	if (logfile_active && logfile_active->value)
	{
		...
		if (!logfile)
		{
			Com_sprintf (name, sizeof(name), "%s/%s.log", FS_Gamedir (), sv_logfile->string);
			if (sv_logfilebackup->value)
			{
				Com_sprintf (newname, sizeof(newname), "%s/%s_%s.log", FS_Gamedir(), sv_logfile->string, datebuf, timebuf);
				rename(name,newname);
			}
...
==================
*/
void MSG_WriteDeltaEntity (entity_state_t *from, entity_state_t *to, sizebuf_t *msg, int force, int newentity)
{
	int		bits;
...
void Qcommon_Init (int argc, char **argv)
{
	...
	// ************* NiceAss **************
	// Modifed version string to include "NCServer" at the end.
	s = va("%4.2f %s %s %s %s", VERSION, CPUSTRING, __DATE__, BUILDSTRING, "NCServer v0.3");
	...
	*/
	Com_Printf ("Version:\n\n");
	Com_Printf ("       %s %4.2f%s.%i\n", GAMENAME, VERSION, BUILD);
	Com_Printf ("       %s %s\n\n", CPUSTRING, BUILDSTRING);
	Com_Printf ("Built:\n\n");
	Com_Printf ("       %s %s\n\n", BUILDSTAMP);
	Com_Printf ("Developers:\n\n");
	Com_Printf ("       %s\n       %s\n       %s\n       %s\n\n", DEVELOPERS);
}
